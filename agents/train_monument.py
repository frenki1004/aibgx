"""
Training manager for monument_example.py.
...
Usage:
  python3 agents/train_monument.py

  # Start from a specific opponent index (0-based):
  python3 agents/train_monument.py --start-opp=1
...
HISTORY_FILE     = os.path.join(AGENTS_DIR, "monument_history.json")

OPPONENTS        = ["aggressive", "econ", "smarter"]
POLL_INTERVAL    = 3
SERVER_BOOT_WAIT = 3
EVAL_EVERY       = 5   # must match EVAL_EVERY in monument_example.py
...
def start_bot(opponent: str) -> subprocess.Popen:
    env  = {**os.environ, "TEAM": "0", "BOT_NAME": "MonumentBot", "OPPONENT": opponent}
    proc = subprocess.Popen(
        [sys.executable, "monument_example.py"],
    ...
    try:
        while True:
            ...
            if bot_proc.poll() is not None:
                print("[train] MonumentBot crashed — restarting...")
                bot_proc = start_bot(opp_name)
