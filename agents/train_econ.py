...
Usage:
  ...
  python3 agents/train_econ.py --games=50
...
Opponent rotation order (repeats):
  smarter → aggressive → hybrid → (repeat)
"""
...
BEST_FILE        = os.path.join(AGENTS_DIR, "econ_best_weights.json")

OPPONENTS        = ["aggressive", "hybrid", "smarter"]
...
def start_bot(opponent: str = "unknown") -> subprocess.Popen:
    print(f"[train] Starting EconBot (team 0, opponent={opponent})...")
    ...
    print(f"[train] EconBot running (pid {proc.pid})")
    return proc
    ...
    try:
        while True:
            ...
            if games_vs_opp >= games_per_opp and current % EVAL_EVERY == 0:
                ...
                opp_proc = None
                opp_idx += 1
                stop(bot_proc, "EconBot (rotation)")
