...
function encodeStateForNN(state, playerId) {
  ...
  const enemyMonuments = monuments.filter(
    (m) => m.controlledBy !== null && m.controlledBy !== playerId
  ).length;
  ...
  let myDistToMonument = W,
    enemyDistToMonument = W;
  if (monuments.length > 0) {
    if (myUnits.length > 0)
      myDistToMonument = Math.min(
        ...myUnits.flatMap((u) => monuments.map((m) => chebyshevDistance(u.x, u.y, m.x, m.y)))
      );
    if (enemyUnits.length > 0)
      enemyDistToMonument = Math.min(
        ...enemyUnits.flatMap((u) => monuments.map((m) => chebyshevDistance(u.x, u.y, m.x, m.y)))
      );
  }
...
function encodeVisitsForNN(rootVisits, selectedActions, state, playerId) {
  ...
  // -1 = padding (no unit/city in this slot) — ignored by loss function
  const buildDecisions = new Array(MAX_CITIES).fill(-1);
  const moveDecisions = new Array(MAX_UNITS).fill(-1);

  // Mark real slots as 0 (default "nothing"/"stay"), overwritten below if action exists
  for (let i = 0; i < myCities.length && i < MAX_CITIES; i++) buildDecisions[i] = 0;
  for (let i = 0; i < myUnits.length && i < MAX_UNITS; i++) moveDecisions[i] = 0;
  let expandCount = 0;
  let buildCity = 0;
